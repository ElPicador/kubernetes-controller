...
# Greatly inspired by https://github.com/k8spatterns/examples/blob/master/advanced/Controller/config-watcher-controller.sh.
...
start_event_loop() {
  ...
  do
    ...
    if [[ "${annotations}" != "null" ]]; then
      pod_selector=$(echo ${annotations} | jq -r 'to_entries | .[] | select(.key == "k8spatterns.io/podDeleteSelector") | .value')
    fi
